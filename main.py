...
def generate_test():
    ...
    try:
        ...
        print(f"Note: Could not read source file for {subject}/{unit}/{topic}. Will generate all from general knowledge.")

    if context_text:
        # Case 1: Context exists. Generate 70% from context, 30% from general knowledge.
        context_q_count = math.ceil(num_questions * 0.7)
        general_q_count = num_questions - context_q_count

        if context_q_count > 0:
            prompt_context = PROMPT_TEMPLATE.format(num_questions=context_q_count, language=language, topic=f"{topic} (from {unit})", context=context_text)
            all_questions.extend(generate_questions_for_topic(prompt_context))

        if general_q_count > 0:
            prompt_general = PROMPT_TEMPLATE.format(num_questions=general_q_count, language=language, topic=f"{topic} (from {unit})", context="No specific context provided for these questions. Generate based on general knowledge of the topic.")
            all_questions.extend(generate_questions_for_topic(prompt_general))
        
        random.shuffle(all_questions)
    else:
        # Case 2: No context file. Generate all questions from general knowledge.
        prompt = PROMPT_TEMPLATE.format(num_questions=num_questions, language=language, topic=f"{topic} (from {unit})", context="No specific context provided. Generate questions based on general knowledge of the topic.")
...
def generate_mock_test():
    ...
    for subject, details in MOCK_TEST_STRUCTURE.items():
        ...
        for topic, num_questions in details['topics'].items():
            print(f"Attempting to generate {num_questions} questions for {subject} -> {topic}...")
            context_text = None
            try:
                # Mock test topics might not map 1:1 to folder names, so we search.
                # Example: "History, Culture of India..." might be in a "History" folder.
                # This logic tries to find any .txt file in a folder that starts with the topic name.
                    ...
                    for filename in os.listdir(topic_path):
                        if filename.endswith('.txt'):
                            with open(os.path.join(topic_path, filename), 'r', encoding='utf-8') as f:
                                context_text = f.read()
                            print(f"Found context file for '{topic}': {filename}")
                            break # Use the first file found
            except Exception as e:
                print(f"Error while searching for context for {topic}: {e}")
            ...
            if context_text:
                ...
                if generated_qs:
                    ...
                    print(f"Warning: Failed to generate questions for topic '{topic}' even with context.")
            else:
                print(f"Warning: No context file found for mock test topic '{topic}'. SKIPPING question generation.")
    
    if len(all_questions) < 150: # Lowered threshold as some source files might be missing
         return jsonify({"error": f"Failed to generate a complete test as some source files were missing. Only got {len(all_questions)}/200 questions. Please check the source material and try again."}), 500
