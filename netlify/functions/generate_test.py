...
def generate_test():
    ...
    if not model:
        return jsonify({"error": "Gemini API is not configured. Check the function logs and API Key."}), 500
    ...
    file_path = "" # Initialize for error message
    try:
        ...
        return jsonify({"error": f"Could not read source file. Check if 'included_files' is set in netlify.toml.", "details": str(e), "path_checked": file_path}), 404
    ...
    try:
        ...
        return jsonify({"error": f"An error occurred while generating questions with the AI: {e}"}), 500
