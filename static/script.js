document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchMockTestBlueprint() {
        try {
            ...
            if (!response.ok) throw new Error('Failed to load mock test structure.');
            appState.mockTestBlueprint = await response.json();
        } catch (error) {
            ...
            try {
                ...
                const response = await fetch('/api/generate-test', {
                    ...
                    body: JSON.stringify({
                        ...
                        unit: null // Mock test topics don't map to units, so backend will use general knowledge
                    }),
                });

                if (!response.ok) {
                    const errorData = await response.json();
                    throw new Error(errorData.error || `Failed on topic: ${task.topic}.`);
                }
