document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleSubjectSelection(subject) {
        ...
        // Always ask for language when General Studies is selected.
    ...
    function renderCurrentTopicsPage() {
        ...
        // Re-add event listeners for the current page of topics
    ...
    async function startMockTest() {
        ...
        try {
            ...
            appState.testQuestions = await response.json();
            if (appState.testQuestions.length < 150) {
                throw new Error(`The AI failed to generate a complete mock test, likely due to missing source files. Only got ${appState.testQuestions.length} questions. Please try again later.`);
            }
            const duration = 180 * 60; // 3 hours
            renderTest(duration, "Full Mock Test");
